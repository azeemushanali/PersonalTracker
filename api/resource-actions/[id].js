...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      const actions = await db.collection('resource_actions')
        .find({ userId: id })
        .sort({ createdAt: -1 })
        .toArray();
    ...
    if (req.method === 'PUT') {
      ...
      const result = await db.collection('resource_actions').findOneAndUpdate(
        { _id: new ObjectId(id) },
        updateDoc,
    ...
    if (req.method === 'DELETE') {
      const result = await db.collection('resource_actions').deleteOne({ 
        _id: new ObjectId(id) 
      });
